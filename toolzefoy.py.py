...
class Zefoy:
    ...
    def get_captcha(self):
        ...
        request = self.session.get(self.base_url, headers=self.headers, timeout=60)
        ...
        try:
            ...
            request = self.session.get(f"{self.base_url}{captcha_url}",headers=self.headers, timeout=60)
    ...
    def send_captcha(self, new_session = False):
        ...
        request = self.session.post(self.base_url, headers=self.headers, data=self.captcha_, timeout=60)
    ...
    def solve_captcha(self, path_to_file = None, b64 = None, delete_tag = ['\n','\r']):
        if path_to_file: 
            ...
            task = 'temp.png'
        request = self.session.post('https://api.ocr.space/parse/image?K87899142388957', headers={'apikey':'K87899142388957'}, files={'task':open(task,'rb')}, timeout=60).json()
    ...
    def get_table(self, i = 1):
        ...
        for service in self.services:
            ...
            i += 1
        table.title = f"{Fore.WHITE} BUFF TIKTOK BY HAITOOL {len([x for x in self.services_status if self.services_status[x]])}{Fore.RESET}"
        print(table)
        ...
        while True:
            ...
            if 'Session expired. Please re-login' in self.video_info:
                ...
                while time.time()-2<_+zefoy:
                    t-=1
                    print('\033[31m[HaiTool]=>Please what: \033[1;32m{0}\033[31m '.format(t)+'second\033[1;32m',end='\r')
                    time.sleep(1)
                continue
    ...
    def get_video_info(self):
        request = self.session.get(f'https://tiktok.livecounts.io/video/stats/{urlparse(self.url).path.rpartition("/")[2]}',headers={'authority':'tiktok.livecounts.io','origin':'https://livecounts.io','user-agent':self.headers['user-agent']}, timeout=60).json()
    ...
    def get_video_id(self, url_ = None, set_url=True):
        ...
        if url.isdigit():
            self.url = url_
            return url_
        request = requests.get(f'https://api.tokcount.com/?type=videoID&username=https://vm.tiktok.com/{url}',headers={'origin': 'https://tokcount.com','authority': 'api.tokcount.com','user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Safari/537.36'}, timeout=60)
        if request.text == '':
